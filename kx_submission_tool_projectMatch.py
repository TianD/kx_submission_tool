...
class ProjNameMatch(object):
    """检查文件名称是否符合预设的项目规则"""
    def __init__(self):
        super(ProjNameMatch, self).__init__()
        self.fileName = ''                                            # 文件名称
        ...
        rules = {
            ...
            'SENBA':
                    [
                        ...
                        ['SB|SENBA', '999|000', '\w+', '(sc){0,1}[0-9]+[a-zA-Z]{0,2}', '\\w+', links[0], '(c){0,1}[0-9]+']
                    ],
            'GUOXUE':
                    [
                        ['GX|GUOXUE', '(?!999|000)[0-9]+', '(sq){0,1}[0-9]+[a-zA-Z]{0,2}', '(sc){0,1}[0-9]+[a-zA-Z]{0,2}', '\\w+', links[0], '(c){0,1}[0-9]+'],
                        ['GX|GUOXUE', '999|000', '[a-zA-Z]+[0-9]{0,2}', '(sc){0,1}[0-9]+[a-zA-Z]{0,2}', '\\w+', links[0], '(c){0,1}[0-9]+']
                    ],
            'ROCK':
                    [
                       ['RK|ROCK', '(?!999|000)[0-9]+', '(sq){0,1}[0-9]+[a-zA-Z]{0,2}', '(sc){0,1}[0-9]+[a-zA-Z]{0,2}', '\\w+', links[0], '(c){0,1}[0-9]+'],
                       ['RK|ROCK', '999|000', '[a-zA-Z]+[0-9]{0,2}', '(sc){0,1}[0-9]+[a-zA-Z]{0,2}', '\\w+', links[0], '(c){0,1}[0-9]+']                       
                    ]
        }
        ...
        self._matchsDict = {
            ...
            'SENBA':
                [
                    ...
                    groups[1] % self._crossMerger(self._keys[0], rules['SENBA'][2]),
                ],
            'GUOXUE':
                [
                    groups[1] % self._crossMerger(self._keys[0], rules['GUOXUE'][0]),
                    groups[1] % self._crossMerger(self._keys[0], rules['GUOXUE'][1])
                ],
            'ROCK':
                [
                    groups[1] % self._crossMerger(self._keys[0], rules['ROCK'][0]),
                    groups[1] % self._crossMerger(self._keys[0], rules['ROCK'][1])                    
                ]
        }
        ...
        self._localPathDict = {
            ...
            'GUOXUE': [self._keys[0][3], (self._keys[0][1], self._keys[0][2])],
            'ROCK': [self._keys[0][3], (self._keys[0][1], self._keys[0][2])]
        }
        ...
        self._serverPathDict = {
            'XFTL': {
                ...
                'Comp':['//kaixuan.com/kx/Proj/Priject/xuanfengtuoluo/Production/Render/Comp/', parityList[0], localPathList[0]]
            },
            'SENBA': {
                ...
                'Comp':['//kaixuan.com/kx/Proj/SENBA/Production/Render/Comp/', parityList[0], localPathList[0]]
            },
            ### 有可能要改路径, 问凯欣
            'GUOXUE': {
                'Images': ['//kaixuan.com/kx/Proj/GUOXUE/Production/Render/Images/', parityList[0], localPathList[0]],
                'Mov': ['//kaixuan.com/kx/Proj/GUOXUE/Production/Render/Mov/', parityList[0], localPathList[0]],
                'Comp':['//kaixuan.com/kx/Proj/GUOXUE/Production/Render/Comp/', parityList[0], localPathList[0]]
            },
            'ROCK':{
                'Images': ['//kaixuan.com/kx/Proj/ROCK/Production/Render/Images/', parityList[0], localPathList[0]],
                'Mov': ['//kaixuan.com/kx/Proj/ROCK/Production/Render/Mov/', parityList[0], localPathList[0]],
                'Comp':['//kaixuan.com/kx/Proj/ROCK/Production/Render/Comp/', parityList[0], localPathList[0]]  
            }
    ...
    def setPrefix(self, mod=1, custom={}):
        ...
        如: self.setPrefix(custom = {'project_name':'aaa'})
        key: 'project_name', 'episode_number', 'session_number', 'scene_number', 'scene_describe', 'process_name', 'version_number'
        """
...
def uploadMaterial():
    """将渲染好的素材文件,上传到指定的服务器路径"""
    path = ''
    serverPath = ''
    fileName = ''
    singleFilter = "All Files (*.*)"
    caption = u'上传渲染素材到服务器路径 (请注意,路径不要有中文,文件夹不能存在空格!)'
    paths = cmds.fileDialog2(fileFilter=singleFilter, dialogStyle=2 ,fileMode=3,cap=caption)
    if paths:
        path = paths[0]
        fileName = os.path.basename(path)
    projClass = ProjNameMatch()
    projClass.setFileName(fileName)
    serverPath = projClass.getUploadServerPath()
    if serverPath:
        serverPath = serverPath.replace('/','\\') + '\\'
        path = path.replace('/','\\')
        convertCmd = 'xcopy {0} {1} /e /y'
        os.system(convertCmd.format(path,serverPath))
    pass